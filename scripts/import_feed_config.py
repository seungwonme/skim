...
def load_feed_config(feed_config_path: Path) -> dict[str, str]:
    """Load the YOUTUBE_CHANNELS mapping from feed_config.py."""
    spec = importlib.util.spec_from_file_location("feed_config_import", feed_config_path)
    if spec is None or spec.loader is None:
        raise RuntimeError(f"feed_config.py를 읽을 수 없습니다: {feed_config_path}")

    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)

    channels = getattr(module, "YOUTUBE_CHANNELS", None)
    if not isinstance(channels, dict):
        raise RuntimeError("feed_config.py에 YOUTUBE_CHANNELS dict가 없습니다.")

    return channels


def preview_import(db_path: Path, feed_config_path: Path) -> dict:
    """Return the import preview without mutating the database."""
    init_db(db_path)
    channels = load_feed_config(feed_config_path)
...
def run_import(db_path: Path, feed_config_path: Path) -> dict:
    """Insert missing YouTube channels into tracked_sources."""
    preview = preview_import(db_path, feed_config_path)
    conn = get_connection(db_path)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--feed-config",
        type=Path,
        default=Path("feed_config.py"),
        help="Path to feed_config.py.",
    )
    parser.add_argument(
        "--preview",
        action="store_true",
...
def main() -> int:
    ...
    result = (
        preview_import(args.db, args.feed_config)
        if args.preview
        else run_import(args.db, args.feed_config)
    )
