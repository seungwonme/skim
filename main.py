...
사용법:
    ...
    python main.py crawl all --days 1 --no-content
    python main.py crawl threads --user-id 314216
...
from src.db import finish_run, init_db, save_posts, save_run
...
def crawl(
    ...
    dry_run: bool = typer.Option(False, "--dry-run", help="미리보기 (저장 안함)"),
    ...
    예시:
        ...
        python main.py crawl hackernews geeknews --days 1
    """
    if not platforms:
        platforms = ["all"]
    ...
    for platform in targets:
        ...
        if not dry_run:
            if output and len(targets) == 1:
                ...
                filepath = f"data/{platform}/{timestamp}.json"
...
@app.command()
def login(
    platform: str = typer.Argument("threads", help="로그인할 플랫폼 (threads, x, linkedin)"),
):
    ...
    """
    from src.crawlers.auth.cdp import login as cdp_login  # pylint: disable=import-outside-toplevel
