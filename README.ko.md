...
Python 3.12 이상이 필요합니다. 태스크는 `just`, Python tooling은 `uv`, git hook용 Node tooling은 `pnpm`을 사용합니다.
...
just install-desktop                 # /Applications/Skim.app 설치
scripts/build-app.sh ~/Applications  # 지정 폴더에 설치
```
...
```bash
just lint
just test     # Python pytest + Swift unit test
just e2e      # fixture DB + 실제 desktop app boot smoke
just build
just dev
